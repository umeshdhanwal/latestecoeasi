...
export default function EcoEasiOverviewSection() {
  ...
  const handleTabClick = (path) => {
    if (path === "/") {
      navigate(path, { state: { scrollToHomepagetestSection: true } });
    } else {
  ...
  return (
    <>
      ...
      <div id="ecoEasiOverview" className="mt-[90px] flex flex-col items-center">
        <div className="container-xs flex flex-col gap-[120px] lg:gap-[90px] lg:px-5 md:gap-[90px] md:px-5 sm:gap-[60px]">
          ...
          <TabList className="mx-[274px] flex flex-wrap justify-between gap-[50px] md:mx-0">
            <Tab 
              className="px-[164px] py-3.5 text-[18px] font-bold capitalize text-green-500_02 border-2 border-green-500_02 rounded-full lg:px-8 lg:text-[15px] md:px-5 sm:px-4"
              onClick={() => handleTabClick("/")}
            >
              SMEs
            </Tab>
            <Tab 
              className="ml-4 px-[164px] py-3.5 text-[18px] font-bold capitalize bg-green-500_02 text-white rounded-full lg:px-8 lg:text-[15px] md:px-5 sm:px-4"
